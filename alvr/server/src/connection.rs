...
use alvr_session::{
    get_profile_config, AveragingStrategy, BitrateMode, ControllersEmulationMode, FetchSide,
    FrameSize, OpenvrConfig, SessionConfig, WindowType,
};
use alvr_sockets::{
...
fn connection_pipeline(
    ...
    *STATISTICS_MANAGER.lock() = Some(StatisticsManager::new(
    ...
    // Initialize the BitrateManager with the computed values.
            ...
            if let Switch::Enabled(http_server) = &server_read_lock.settings().custom.http_server {
                ...
                if fetch_from == FetchSide::Server {
                    while is_streaming(&client_hostname) {
                        ...
                        let sleep_duration = if elapsed >= Duration::from_secs_f32(request_interval)
                        {
                            Duration::ZERO
                        } else {
